...
export default function Revo({ posts, backgroundImage }) {
  return (
    <div
    ...
    >
      ...
      <main className="px-8 py-20 relative z-10">
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          {posts.length === 0 && <p className="text-white/60">No markdown found in RevoTechnologies/revoWriteups</p>}
            ...
            {posts.map((p) => (
              <div key={p.slug} className="project-card">
                ...
                <div className="project-links">
                  <Link href={`/revo/${p.slug}`} className="project-links">
                    Open →
                  </Link>
                </div>
              </div>
...
export async function getStaticProps() {
  const dir = 'RevoTechnologies/revoWriteups'
  ...
  return { props: { posts, backgroundImage: previews.revo } }
}

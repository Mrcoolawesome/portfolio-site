...
export default function Robotics({ posts }) {
  return (
    <div>
      <NavBar />
      <main className="p-8">
        <h1 className="text-2xl mb-4">High School Robotics</h1>
        {posts.length === 0 && <p>No markdown found in RoboticsStuff/roboticsObsidian</p>}
        <div className="grid gap-4">
          {posts.map((p) => (
            <div key={p.slug} className="card">
              <h3 className="text-lg">{p.title}</h3>
              {p.excerpt && <p className="mt-2">{p.excerpt}</p>}
              <Link href={`/robotics/${p.slug}`} className="btn-outline mt-4 inline-block">Open</Link>
            </div>
          ))}
        </div>
      </main>

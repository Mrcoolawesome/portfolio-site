...
async function renderMarkdown(filePath) {
  ...
  const processed = await remark().use(html, { sanitize: false }).process(transformed)
  let contentHtml = processed.toString()

  // Convert YouTube links in rendered HTML to embedded iframes
  // Handle case where it's wrapped in an Anchor
  contentHtml = contentHtml.replace(/<a[^>]*href="(https:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+))"[^>]*>[^<]*<\/a>/gi, (match, url, videoId) => {
  ...
  // Handle case where it's bare text URL, potentially inside <p>
  contentHtml = contentHtml.replace(/(?<!["'])(?:<p>\s*)?(https:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)(?:&[a-zA-Z0-9_=+-]+)*)(?:\s*<\/p>)?/gi, (match, url, videoId) => {
    return `<iframe width="100%" height="500" src="https://www.youtube.com/embed/${videoId}" title="YouTube video" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="max-width: 100%; margin: 2rem auto; display: block; border-radius: 0.75rem;"></iframe>`
  })

  // Add lazy loading to all img tags for better performance
  contentHtml = contentHtml.replace(/<img\s+([^>]*?)>/gi, (match, attrs) => {
    // Only add loading if not already present
    if (/\s+loading\s*=/.test(match)) return match
